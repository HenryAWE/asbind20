...
bool weakref::equals_to(void* ref) const
{
    if(m_ref != ref)
        return false;
...
static void register_weakref_impl(AS_NAMESPACE_QUALIFIER asIScriptEngine* engine)
{
    ...
    auto reset_wrapper = [](weakref& this_, void* ref) -> weakref&
    {
        this_.reset(ref);
        return this_;
    };

    template_value_class<weakref, UseGeneric> c1(engine, "weakref<T>", flags);
    c1
        .default_constructor()
        .template constructor<void*>("T@+")
        .destructor()
        .method("T@ opImplCast()", fp<&weakref::get>)
        .method("T@ get() const", fp<&weakref::get>)
        .method("weakref<T>& opHndlAssign(const weakref<T>&in)", fp<(&weakref::operator=)>)
        .opAssign()
        .opEquals()
        .method("weakref<T>& opHndlAssign(T@)", reset_wrapper)
        .method("bool opEquals(const T@+) const", fp<&weakref::equals_to>);

    template_value_class<weakref, UseGeneric> c2(engine, "const_weakref<T>", flags);
    c2
        .default_constructor()
        .template constructor<void*>("T@+")
        .destructor()
        .method("const T@ opImplCast() const", fp<&weakref::get>)
        .method("const T@ get() const", fp<&weakref::get>)
        .method("const_weakref<T>& opHndlAssign(const const_weakref<T>&in)", fp<(&weakref::operator=)>)
        .opAssign()
        .opEquals()
    ...
    // Interoperability
    c2
        .method("const_weakref<T> &opHndlAssign(const weakref<T>&in)", fp<(&weakref::operator=)>)
        .method("bool opEquals(const weakref<T>&in) const", fp<(&weakref::operator==)>);
}
